<template>
  ...
  <div class="game-screen q-mb-auto flex column">
    ...
    <q-list class="rounded-borders column relative-position q-my-auto">
      <template v-if="state.playerList.length !== 0">
        <div
        ...
        >
          <div
            class="item-wrapper"
            v-for="playerHolder in totalPlayerList"
            :key="playerHolder.index"
            style="
              border-width: 2px;
              border-style: solid none;
              border-color: #9e9e9e;
        ...
        >
          <BounceInTransitonGroup>
            <div
              class="item-wrapper"
              v-for="(player, index) in state.playerList"
              :key="player.uuid"
              style="
                border-width: 2px;
                border-style: solid none;
              ...
              >
                ...
                <q-item-section>
                  ...
                  <ToggleReadyTransition
                    ...
                    v-else
                    :name="
                      state.readyPlayerList.includes(player.uuid)
                        ? 'stamp-ready'
                        : 'stamp-preparing'
                    "
                  >
                    <div
                      v-if="state.readyPlayerList.includes(player.uuid)"
  ...
  <!-- Bottom Toolbar -->
  <div
    class="bottom-tool-bar flex justify-between items-center q-mb-lg"
    style="margin-top: 100px"
  >
    <!-- Invite button -->
    <div style="flex: 0 1 33.3%">
      <div
        class="invite q-px-sm q-py-xs cursor-pointer q-gutter-xs items-center flex"
    ...
    <div style="flex: 0 1 33.3%">
        ...
        :class="[
          {
            'animated bounce infinite slower':
              isAllPlayersReady && !isGameStarting,
          },
          !isGameStarting ? 'cursor-pointer' : 'start-button--active disable',
        ]"
      ...
      <div
        ...
        @click="!isGameStarting && toggleStatus()"
      >
        Ready
      </div>
    </div>
    ...
    <div style="flex: 0 1 33.3%">
      <div class="flex q-px-sm items-center q-gutter-xs">
        <q-icon name="group" size="2rem" />
        <span class="playercount text-h6">
          {{
            `${state.playerList.length}/${state.gameSettings.numberOfPlayers}`
          }}
        </span>
      </div>
    </div>
...
const state = reactive({
  gameSettings: {
    gameId: "6a938638-ee5b-41d8-9d4d-333c894d66a5",
    gameCode: "jkrb",
    gameMode: "pictionary",
    isPrivate: false,
    streamer: false,
    numberOfPlayers: 5,
    rounds: 3,
    hostUUID: "e3bb90d0-f104-4ba1-a541-ce74e99259ad",
  },
  currentPlayer: {
    uuid: "xxxxx",
    displayName: "Jason",
...
// STATIC DATA
const totalPlayerList = ref([]);
const currentPlayerUUID = state.currentPlayer.uuid;
const isCurrentPlayerReady = false;
const isHost = state.gameSettings.hostUUID === currentPlayerUUID;
const isGameStarting = computed(() => state.countdown !== null);
const isAllPlayersReady = computed(
  () => state.gameSettings.numberOfPlayers - 1 === state.readyPlayerList.length
);
// QR Code
const preLobbyURL = `${window.location.origin}/join?code=${state.gameSettings.gameCode}`;
...
onMounted(() => {
  ...
  setTimeout(() => {
    state.playerList = [
      { displayName: "John", uuid: "e3bb90d0-f104-4ba1-a541-ce74e99259ad" },
      { displayName: "Jason", uuid: "xxxxx" },
      { displayName: "Zhang", uuid: "123" },
    ];
    state.readyPlayerList = ["xxxxx", "123"];
  }, 1000);
});
const enterLobby = () => {
  state.playerList = [
  ...
  totalPlayerList.value = Array.from(
    { length: state.gameSettings.numberOfPlayers },
    (_, i) => {
      if (i === 0) {
        return {
          index: i,
          uuid: state.gameSettings.hostUUID,
...
const toggleStatus = () => {
  const isCurrentPlayerReady = state.readyPlayerList.includes(
    state.currentPlayer.uuid
  );
  if (isCurrentPlayerReady) {
    state.readyPlayerList = state.readyPlayerList.filter(
      (playerUUID) => playerUUID !== state.currentPlayer.uuid
    );
  } else {
    state.readyPlayerList.push(state.currentPlayer.uuid);
  }
};
const startGame = () => {
  // check all players are ready
  //   const isAllPlayersReady = state.playerList
  //     .slice(1)
  //     .every(({ uuid }) => state.readyPlayerList.includes(uuid));
  //   if (state.readyPlayerList.length === 0 || !isAllPlayersReady) return;
  //   startGame();
  // Add michael to game
  state.playerList = [
    { displayName: "John", uuid: "e3bb90d0-f104-4ba1-a541-ce74e99259ad" },
    { displayName: "Michael", uuid: "e3bb90d0-f104-4ba1-a541" },
    { displayName: "Alex", uuid: "e3bb90d0-f104-4ba1-a541-dfdfdfe" },
  ];
  setTimeout(() => {
    state.playerList = [
      { displayName: "John", uuid: "e3bb90d0-f104-4ba1-a541-ce74e99259ad" },
  ...
  //   const updatedPlayer = Object.assign({}, playerToJoin, {
  //     uuid: "sdsdaDsdsada-sdsadsda",
  //     displayName: "Michael",
  //   });
  //   state.playerList[updatedPlayer.index] = updatedPlayer;
  //   console.log(state.playerList, "list");
};
</script>
